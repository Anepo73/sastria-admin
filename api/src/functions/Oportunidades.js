...
const BACKEND_URL = "https://fn-sastri-graph-proxy1-cufvhzhsc6b0f0dq.brazilsouth-01.azurewebsites.net/api/Oportunidades";
...
app.http('Oportunidades', {
    ...
    handler: async (request, context) => {
        ...
        try {
            ...
            // Tenant ID: pode vir do header ou do JWT
            const tenantId = request.headers.get('x-tenant-id') || jwtPayload.tenantId;
            if (!tenantId) {
                return { status: 400, headers: { 'Content-Type': 'application/json', ...cors }, jsonBody: { ok: false, error: 'x-tenant-id ausente.' } };
            }
            ...
            context.error("Proxy Oportunidades errored:", error);
